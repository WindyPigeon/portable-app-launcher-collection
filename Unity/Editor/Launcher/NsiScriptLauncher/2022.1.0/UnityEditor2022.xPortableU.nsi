...
Section "Main"
    ...
    ; AdjustPaths:
        ...
        ${IfNot} ${FileExists} "$SETTINGSDIRECTORY\com.unity3d.UnityEditor5.x.reg"
            ...
            WriteINIStr "$SETTINGSDIRECTORY\com.unity3d.UnityEditor5.x.reg" "HKEY_CURRENT_USER\Software\Unity Technologies\Unity Editor 5.x" '"AssetStoreCacheRootPath"' '"$0/Library/Unity"'
        ${EndIf}
        ...
        ${IfNot} ${FileExists} "$DATADIRECTORY\.upmconfig.toml"
            ...
            ${WordReplace} "$DATADIRECTORY" "\" "\\" + $0
            ${ConfigWrite} "$DATADIRECTORY\.upmconfig.toml" "cacheRoot = " '"$0\\Library\\Unity\\cache"' $0
        ${EndIf}
        ...
        ${AndIf} $LASTDIRECTORY != $CURRENTDIRECTORY
            ...
            ${WordReplace} "$LASTDIRECTORY" "\" "\\" + $0
            ${WordReplace} "$EXEDIR" "\" "\\" + $1
            ${ReplaceInFile} "$DATADIRECTORY\.upmconfig.toml" "$LASTDRIVE\\$LASTDIRECTORY" "$EXEDIR"
        ${EndIf}
        ...
        ${AndIf} $LASTPORTABLEAPPSBASEDIRECTORY != $PORTABLEAPPSBASEDIRECTORY
            ...
            ${WordReplace} "$LASTPORTABLEAPPSBASEDIRECTORY" "\" "\\" + $0
            ${WordReplace} "$PORTABLEAPPSBASEDIRECTORY" "\" "\\" + $1
            ${ReplaceInFile} "$DATADIRECTORY\.upmconfig.toml" "$LASTDRIVE\\$0" "$CURRENTDRIVE\\$1"
        ${EndIf}
        ...
        ${AndIf} $LASTDRIVE != $CURRENTDRIVE
            ...
            ${WordReplace} "$LASTPORTABLEAPPSBASEDIRECTORY" "\" "\\" + $0
            ${WordReplace} "$PORTABLEAPPSBASEDIRECTORY" "\" "\\" + $1
            ${ReplaceInFile} "$DATADIRECTORY\.upmconfig.toml" "$LASTDRIVE\\" "$CURRENTDRIVE\\"
        ${EndIf}

    ; BackupOriginalData:
        ;=== Backup original app data
        ;=== Backup original editor preferences, REST certificate and Bee cache
        ...
        ;=== Backup original browser cache and cookies and original editor data files
        ...
        ;=== Backup original Unity's Package Manager configuration
        Rename "$USERPROFILEPATH\.upmconfig.toml" "$USERPROFILEPATH\.upmconfig.toml.BackupBy${NAME}"
    ...
    ; RestoreData:
        ...
        ;=== Restore the Unity's Package Manager configuration
        Rename "$DATADIRECTORY\.upmconfig.toml" "$USERPROFILEPATH\.upmconfig.toml"
        ${If} ${Errors}
            CreateDirectory "$USERPROFILEPATH"
            CopyFiles /SILENT "$DATADIRECTORY\.upmconfig.toml" "$USERPROFILEPATH"
        ${EndIf}
    ...
    ; SetOriginalKeyBack:
        ...
        ;=== Save the Bee cache back
        ClearErrors
        Rename "$APPDATA\Unity\cache" "$DATADIRECTORY\Library\Unity\cache"
        ${If} ${Errors}
            RMDir /r "$DATADIRECTORY\Library\Unity\cache\bee"
            CreateDirectory "$DATADIRECTORY\Library\Unity\cache"
            CopyFiles /SILENT "$APPDATA\Unity\cache\*.*" "$DATADIRECTORY\Library\Unity\cache"
            RMDir /r "$APPDATA\Unity\cache"
        ${EndIf}
        ...
        ;=== Move the Unity's Package Manager configuration back
        ClearErrors
        Rename "$USERPROFILEPATH\.upmconfig.toml" "$DATADIRECTORY\.upmconfig.toml"
        ${If} ${Errors}
            Delete "$DATADIRECTORY\.upmconfig.toml"
            CreateDirectory "$DATADIRECTORY"
            CopyFiles /SILENT "$USERPROFILEPATH\.upmconfig.toml" "$DATADIRECTORY"
            Delete "$USERPROFILEPATH\.upmconfig.toml"
        ${EndIf}
        ...
        Rename "$USERPROFILEPATH\.upmconfig.toml.BackupBy${NAME}" "$USERPROFILEPATH\.upmconfig.toml"

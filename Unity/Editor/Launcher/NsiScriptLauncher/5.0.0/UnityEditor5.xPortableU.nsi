...
Section "Main"
    ...
    ; BackupOriginalData:
        ...
        Rename "$APPDATA\Unity\Asset Store-5.x" "$APPDATA\Unity\Asset Store-5.x.BackupBy${NAME}"

        ;=== Backup original caches folder
        Rename "$APPDATA\Unity\Caches" "$APPDATA\Unity\Caches.BackupBy${NAME}"

        ;=== Backup certificates
        Rename "$APPDATA\Unity\Certificates" "$APPDATA\Unity\Certificates.BackupBy${NAME}"

        ;=== Backup original editor preferences
        Rename "$APPDATA\Unity\Editor-5.x\Preferences" "$APPDATA\Unity\Editor-5.x\Preferences.BackupBy${NAME}"

        ;=== Backup original packages
        Rename "$APPDATA\Unity\Packages" "$APPDATA\Unity\Packages.BackupBy${NAME}"
    ...
    ; RestoreData:
        ...
        ${If} ${Errors}
            ...
            CopyFiles /SILENT "$DATADIRECTORY\Library\Application Support\Unity\Unity_v5.x.ulf" "$COMMONPROGRAMDATA\Unity"
            CopyFiles /SILENT "$DATADIRECTORY\Library\Application Support\Unity\Unity_v5.x.ulf.backup" "$COMMONPROGRAMDATA\Unity"
        ${Else}
            Rename "$DATADIRECTORY\Library\Application Support\Unity\Unity_v5.x.ulf.backup" "$COMMONPROGRAMDATA\Unity\Unity_v5.x.ulf.backup"
        ${EndIf}
        ...
        ${Else}
            CreateDirectory "$APPDATA\Unity\Asset Store-5.x"
        ...
        ${GetRoot} "$DATADIRECTORY\Library\Unity\Certificates" $0
        ${GetRoot} "$APPDATA\Unity\Certificates" $1
        ${If} $0 == $1
            Rename "$DATADIRECTORY\Library\Unity\Certificates" "$APPDATA\Unity\Certificates"
        ${Else}
            CreateDirectory "$APPDATA\Unity\Certificates"
            CopyFiles /SILENT "$DATADIRECTORY\Library\Unity\Certificates\*.*" "$APPDATA\Unity\Certificates"
        ${EndIf}
    ...
    ; SetOriginalKeyBack:
        ...
        ;=== Move the certificates back
        ClearErrors
        Rename "$APPDATA\Unity\Certificates" "$DATADIRECTORY\Library\Unity\Certificates"
        ${If} ${Errors}
            RMDir /r "$DATADIRECTORY\Library\Unity\Certificates"
            CreateDirectory "$DATADIRECTORY\Library\Unity\Certificates"
            CopyFiles /SILENT "$APPDATA\Unity\Certificates\*.*" "$DATADIRECTORY\Library\Unity\Certificates"
            RMDir /r "$APPDATA\Unity\Certificates"
        ${EndIf}
        ...
        ${If} ${Errors}
            Delete "$DATADIRECTORY\Library\Application Support\Unity\Unity_v5.x.ulf"
            Delete "$DATADIRECTORY\Library\Application Support\Unity\Unity_v5.x.ulf.backup"
        ...
        Rename "$APPDATA\Unity\Asset Store-5.x.BackupBy${NAME}" "$APPDATA\Unity\Asset Store-5.x"
        Rename "$APPDATA\Unity\Caches.BackupBy${NAME}" "$APPDATA\Unity\Caches"
        Rename "$APPDATA\Unity\Certificates.BackupBy${NAME}" "$APPDATA\Unity\Certificates"
        Rename "$APPDATA\Unity\Editor-5.x\Preferences.BackupBy${NAME}" "$APPDATA\Unity\Editor-5.x\Preferences"
        Rename "$APPDATA\Unity\Packages.BackupBy${NAME}" "$APPDATA\Unity\Packages"

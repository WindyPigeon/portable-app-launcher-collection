...
!define AppID "UnityEditor3.xPortable"
...
Section "Main"
    ...
    ; AdjustPaths:
        ...
        ${If} ${FileExists} "$SETTINGSDIRECTORY\com.unity3d.UnityEditor3.x.reg"
            ${If} $LASTDIRECTORY != ""
            ${AndIf} $LASTDIRECTORY != $CURRENTDIRECTORY
                ${ReplaceInFile} "$SETTINGSDIRECTORY\com.unity3d.UnityEditor3.x.reg" "$LASTDRIVE/$LASTDIRECTORY" "$EXEDIR"
                ...
                ${While} $7 < $8
                    ...
                    IntOp $7 $7 + 2
                ${EndWhile}
                ${ReplaceInFile} "$SETTINGSDIRECTORY\com.unity3d.UnityEditor3.x.reg" "$1" "$6"
            ${EndIf}

            ${If} $LASTPORTABLEAPPSBASEDIRECTORY != ""
            ${AndIf} $LASTPORTABLEAPPSBASEDIRECTORY != $PORTABLEAPPSBASEDIRECTORY
                ${ReplaceInFile} "$SETTINGSDIRECTORY\com.unity3d.UnityEditor3.x.reg" "$LASTDRIVE/$LASTPORTABLEAPPSBASEDIRECTORY" "$CURRENTDRIVE/$PORTABLEAPPSBASEDIRECTORY"
            ...
            ${AndIf} $LASTDRIVE != $CURRENTDRIVE
                ${ReplaceInFile} "$SETTINGSDIRECTORY\com.unity3d.UnityEditor3.x.reg" "$LASTDRIVE/" "$CURRENTDRIVE/"
    ...
    ; BackupOriginalData:
        ...
        Rename "$APPDATA\Unity\Asset Store-3.x" "$APPDATA\Unity\Asset Store-3.x.BackupBy${NAME}"
        ...
        Rename "$APPDATA\Unity\Editor-3.x\Preferences" "$APPDATA\Unity\Editor-3.x\Preferences.BackupBy${NAME}"
    ...
    ; RestoreData:
        ...
        ${GetRoot} "$DATADIRECTORY\Library\Preferences\Unity\Editor-3.x" $0
        ${GetRoot} "$APPDATA\Unity\Editor-3.x\Preferences" $1
        ${If} $0 == $1
            Rename "$DATADIRECTORY\Library\Preferences\Unity\Editor-3.x" "$APPDATA\Unity\Editor-3.x\Preferences"
        ${Else}
            CreateDirectory "$APPDATA\Unity\Editor-3.x\Preferences"
            CopyFiles /SILENT "$DATADIRECTORY\Library\Preferences\Unity\Editor-3.x\*.*" "$APPDATA\Unity\Editor-3.x\Preferences"
        ${EndIf}

        ;=== Restore the asset store
        ${GetRoot} "$DATADIRECTORY\Library\Unity\Asset Store-3.x" $0
        ${GetRoot} "$APPDATA\Unity\Asset Store-3.x" $1
        ${If} $0 == $1
            Rename "$DATADIRECTORY\Library\Unity\Asset Store-3.x" "$APPDATA\Unity\Asset Store-3.x"
        ${Else}
            CreateDirectory "$APPDATA\Unity\Asset Store-3.x"
            CopyFiles /SILENT "$DATADIRECTORY\Library\Unity\Asset Store-4.x\*.*" "$APPDATA\Unity\Asset Store-3.x"
        ${EndIf}
    ...
    ; RegistryBackup:
        ...
        ${registry::KeyExists} "HKEY_CURRENT_USER\Software\Unity Technologies\Unity Editor 3.x" $0
        ${If} $0 != -1
            ${registry::MoveKey} "HKEY_CURRENT_USER\Software\Unity Technologies\Unity Editor 3.x" "HKEY_CURRENT_USER\Software\PortableApps.com\Keys\HKCU\Software\Unity Technologies\Unity Editor 3.x" $0
        ${EndIf}

        ${registry::KeyExists} "HKEY_CURRENT_USER\Software\PACE Anti-Piracy\InterLok\Reg\Common" $0
        ${If} $0 != -1
            ${registry::MoveKey} "HKEY_CURRENT_USER\Software\PACE Anti-Piracy\InterLok\Reg\Common" "HKEY_CURRENT_USER\Software\PortableApps.com\Keys\HKCU\Software\PACE Anti-Piracy\InterLok\Reg\Common" $0
        ${EndIf}

    ; RestoreTheKey:
        ${registry::RestoreKey} "$SETTINGSDIRECTORY\com.unity3d.UnityEditor3.x.reg" $0
    ...
    ; SaveRegistry:
        ${registry::SaveKey} "HKEY_CURRENT_USER\Software\Unity Technologies\Unity Editor 3.x" "$SETTINGSDIRECTORY\com.unity3d.UnityEditor3.x.reg" "" $0
    ...
    ; CleanupRegistry:
        ${registry::DeleteKey} "HKEY_CURRENT_USER\Software\Unity Technologies\Unity Editor 3.x" $0
    ...
    ; SetOriginalKeyBack:
        ${registry::MoveKey} "HKEY_CURRENT_USER\Software\PortableApps.com\Keys\HKCU\Software\Unity Technologies\Unity Editor 3.x" "HKEY_CURRENT_USER\Software\Unity Technologies\Unity Editor 3.x" $0
        ...
        ${registry::DeleteKeyEmpty} "HKEY_CURRENT_USER\Software\Unity Technologies\Unity Editor 3.x" $0
        ...
        ${registry::DeleteKeyEmpty} "HKEY_CURRENT_USER\Software\PortableApps.com\Keys\HKCU\Software\Unity Technologies\Unity Editor 3.x" $0
        ...
        ClearErrors
        Rename "$APPDATA\Unity\Asset Store-3.x" "$DATADIRECTORY\Library\Unity\Asset Store-3.x"
        ${If} ${Errors}
            RMDir /r "$DATADIRECTORY\Library\Unity\Asset Store-3.x"
            CreateDirectory "$DATADIRECTORY\Library\Unity\Asset Store-3.x"
            CopyFiles /SILENT "$APPDATA\Unity\Asset Store-3.x\*.*" "$DATADIRECTORY\Library\Unity\Asset Store-3.x"
            RMDir /r "$APPDATA\Unity\Asset Store-3.x"
        ${EndIf}
        ...
        ClearErrors
        Rename "$APPDATA\Unity\Editor-3.x\Preferences" "$DATADIRECTORY\Library\Preferences\Unity\Editor-3.x"
        ${If} ${Errors}
            RMDir /r "$DATADIRECTORY\Library\Preferences\Unity\Editor-3.x"
            CreateDirectory "$DATADIRECTORY\Library\Preferences\Unity\Editor-3.x"
            CopyFiles /SILENT "$APPDATA\Unity\Editor-3.x\Preferences\*.*" "$DATADIRECTORY\Library\Preferences\Unity\Editor-3.x"
            RMDir /r "$APPDATA\Unity\Editor-3.x\Preferences"
        ${EndIf}
        ...
        Rename "$APPDATA\PACE Anti-Piracy.BackupBy${NAME}" "$APPDATA\PACE Anti-Piracy"
        Rename "$APPDATA\Unity\Asset Store-3.x.BackupBy${NAME}" "$APPDATA\Unity\Asset Store-3.x"
        Rename "$APPDATA\Unity\Caches.BackupBy${NAME}" "$APPDATA\Unity\Caches"
        Rename "$APPDATA\Unity\Editor-3.x\Preferences.BackupBy${NAME}" "$APPDATA\Unity\Editor-3.x\Preferences"
        
        Rename "$LOCALAPPDATA\PACE Anti-Piracy.BackupBy${NAME}" "$LOCALAPPDATA\PACE Anti-Piracy"
        Rename "$LOCALAPPDATA\Unity\Editor.BackupBy${NAME}" "$LOCALAPPDATA\Unity\Editor"

        Rename "$COMMONPROGRAMDATA\PACE Anti-Piracy.BackupBy${NAME}" "$PACEAPDATADIRECTORY\PACE Anti-Piracy"

        ${If} ${AtLeastWinVista}
            Rename "$PROFILE\AppData\Local\VirtualStore\Program Files (x86)\Common Files\PACE Anti-Piracy.BackupBy${NAME}" "$PROFILE\AppData\Local\VirtualStore\Program Files (x86)\Common Files\PACE Anti-Piracy"
        ${Else}
            Rename "$COMMONFILES\PACE Anti-Piracy.BackupBy${NAME}" "$COMMONFILES\PACE Anti-Piracy"
        ${EndIf}

        ;=== Remove empty folders
        RMDir "$APPDATA\PACE Anti-Piracy"
        RMDir "$APPDATA\Unity\Editor-3.x"
        RMDir "$APPDATA\Unity"
        RMDir "$APPDATA"
